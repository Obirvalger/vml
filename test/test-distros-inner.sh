...
vml run --no-ssh --nproc "$(nproc)" -m 1G fst --exists-replace --running-restart
...
vml run --no-ssh --nproc "$(nproc)" -m 1G --net-tap tap0 --net-address 172.16.0.2/24 \
    --net-gateway 172.16.0.1 tapnet/fst --exists-replace --running-restart
vml run --no-ssh --nproc "$(nproc)" -m 1G --net-tap tap1 --net-address 172.16.0.3/24 \
